...
A window's `minimized` says the window is folded into the taskbar or the Dock — something only the tracker can see, and something a reader needs to dim that row or leave it out of a tiling layout. Only a real `true` counts: a tracker of an older version writes no such field, and anything else reads as an ordinary window — a dimmed row for an open window costs more than an undimmed one for a hidden window.
