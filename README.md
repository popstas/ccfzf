...
2. **pid из хука.** Claude Code умеет сменить id внутри живого процесса
   ...
   `~/.claude/claude-wt/<id>.meta.json` ровно в этот момент, а в нём свой pid.
   Процесс переезжает на названную сессию, прежний id перестаёт считаться
   живым. Пара `pidStarted` + `boot` страхует от переиспользованных номеров.
   Этот довод не портится со временем: пока процесс жив, он пишет именно эту
   сессию. Он же единственный, кто даёт pid сессиям, поднятым как
   `claude -n <имя>` — в argv у них uuid нет вовсе.
3. **Переезд по отметке.** Запасной путь для сессий, поднятых до того, как хук
   научился писать pid: если у соседа по каталогу проекта отметка хука свежее и
   моложе двух минут, процесс переезжает на него. Процессов, улаженных вторым
   доводом, не касается.
